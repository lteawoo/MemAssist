...
### Requirement: memassist SHALL gate LLM-derived directives before activation

memassist SHALL activate or draft LLM-derived directives based on confidence, explicitness, and path resolution rather than activating every model output.

#### Scenario: High-confidence explicit directive is immediately compiled

- **WHEN** the interpreter returns a high-confidence explicit directive with an enforcement level and a resolved project path
- **THEN** memassist SHALL store the directive as active memory
- **AND** memassist SHALL immediately compile the directive into the corresponding project policy entry without requiring a separate approval step
...
The change SHALL include tests that exercise initialization, directive memory creation, policy compilation, retrieval, and the initialized tool's hook execution in a temporary project.
...
- **AND** a protected or sensitive tool operation SHALL produce the expected policy behavior
